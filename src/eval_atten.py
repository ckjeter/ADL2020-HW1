...
import dataset
from train_atten import Encoder, Decoder, Seq2Seq
...
def main(args):
    ...
    model.load_state_dict(torch.load('model/atten.pth'))
    model.eval()

    with open(predictpath, 'w') as w:
        for i, data in enumerate(tqdm(dataloader, total=len(dataloader))):
            summary = model(data, False, args.draw_atten)
            args.draw_atten = False
...
def _parse_args():
    ...
    parser.add_argument('--draw_atten', type=str2bool, default = False)
    parser.add_argument('--input', type=str, default = 'datasets/seq2seq/valid.pkl')
    parser.add_argument('--output', type = str, default = 'predict/predict_atten.jsonl')
